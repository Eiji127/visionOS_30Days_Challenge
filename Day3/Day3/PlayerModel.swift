...
@Observable class PlayerModel {
    ...
    private func observePlayback() {
        ...
        NotificationCenter.default
            ...
            .sink { [weak self] isPlaybackComplete in
                self?._isPlaybackComplete = isPlaybackComplete
            }
            .store(in: &subscriptions)
        ...
        NotificationCenter.default
            ...
            .sink { [weak self] notification in
                guard let result = InterruptionResult(notification) else {
                    return
                }
    ...
    private func createMetadataItems(for video: Video) -> [AVMetadataItem] {
        let mapping: [AVMetadataIdentifier: Any] = [.commonIdentifierTitle: video.title]
        return mapping.compactMap {
            createMetadataItem(for: $0, value: $1)
        }
    }
    ...
    private func addTimeObserver() {
        ...
        timeObserver = player.addPeriodicTimeObserver(forInterval: timeInterval, queue: .main, using: { [weak self] time in
            guard let self = self, let duration = player.currentItem?.duration else {
                return
            }
...
struct InterruptionResult {
    ...
    init?(_ notification: Notification) {
        guard let type = notification.userInfo?[AVAudioSessionInterruptionReasonKey] as? AVAudioSession.InterruptionType,
